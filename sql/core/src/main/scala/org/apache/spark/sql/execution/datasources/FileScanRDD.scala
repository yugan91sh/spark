...
import org.apache.spark.util.{NextIterator, SliceIterator}
...
class FileScanRDD(
  ...
  override def sliceIterator(from: Long, until: Long, split: RDDPartition, context: TaskContext):
  Iterator[InternalRow] = {
    logInfo(s"SliceIterator: from $from until $until")
    val currTaskLocality = context.getLocalProperty("SAMetrics.taskLocality")
    val files = split.asInstanceOf[FilePartition].files
    val currFilePath = if (files.isEmpty) "bucket-read-empty-task" else files.head.filePath
    logInfo(s"SAMetrics=File ${currFilePath} running in task ${context.taskAttemptId()} " +
      s"on executor ${SparkEnv.get.executorId} with locality ${currTaskLocality}")
    val iterator = new SliceIterator[Object](from, until) with AutoCloseable {
      private val inputMetrics = context.taskMetrics().inputMetrics
      private val existingBytesRead = inputMetrics.bytesRead
      ...
      override def realHasNext(): Boolean = {
        // Kill the task in case it has been marked as killed. This logic is from
        // InterruptibleIterator, but we inline it here instead of wrapping the iterator in order
        // to avoid performance overhead.
        context.killTaskIfInterrupted()
      ...
      override def readNext(): Object = currentIterator.next()

      override def realNext(): Object = {
        val nextElement = readNext()
        // TODO: we should have a better separation of row based and batch based scan, so that we
        // don't need to run this `if` for every record.
        val preNumRecordsRead = inputMetrics.recordsRead
        if (nextElement.isInstanceOf[ColumnarBatch]) {
